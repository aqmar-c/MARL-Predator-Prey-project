...
class PredatorsPrey(object):
    ...
    def actor_prey_designed(self):
        # Get the prey's current position
        prey_pos = self.preys_positions[0]

        # Find empty neighbors (safe positions to move)
        [empty_neighbors_pos, action_to_empty_neighbor] = self.empty_neighbor_finder(prey_pos)

        # If there are no safe positions, return STAY or random move as a fallback
        if not empty_neighbors_pos:
            return self.STAY if random.random() < 0.5 else self.actor_prey_random()

        # Calculate distances to the nearest predator for each empty neighbor position
        distances_to_predators = []
        for pos in empty_neighbors_pos:
            distance = min([self.distance(pos, predator_pos) for predator_pos in self.predators_positions])
            distances_to_predators.append(distance)

        # Choose the position that maximizes the distance to the nearest predator
        max_distance_idx = distances_to_predators.index(max(distances_to_predators))
        best_move_position = empty_neighbors_pos[max_distance_idx]

        # Return the corresponding action to move to the best position
        return action_to_empty_neighbor[max_distance_idx]

    def distance(self, pos1, pos2):
        # Simple Euclidean distance for simplicity, but you can use other distance metrics
