...
from keras.layers import Dense, Lambda, Input, Concatenate, Dropout
...
class Brain(object):
    def __init__(self, state_size, action_size, brain_name, arguments):
        ...
        # print("dummy " + str(state_size))

    def build_model(self):

        x = Input(shape=(self.state_size,))
        # design a neural netwrok model Q(s,a)
        hidden1 = Dense(self.num_nodes, activation='relu')(x)
        dropout1 = Dropout(0.2)(hidden1)  # Apply dropout
        hidden2 = Dense(self.num_nodes, activation='relu')(dropout1)
        dropout2 = Dropout(0.2)(hidden2)  # Apply dropout
